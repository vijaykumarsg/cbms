...
<ul>
 ...
 <h1> CHEFS WAITING FOR APPROVAL</h1>
  ...
  <?php
    ...
    $slno=0;
    $sql="SELECT * FROM `chef` WHERE status='Requested'";
    ...
    if($result->num_rows>0)
    {
        while($row=$result->fetch_assoc())
        {
            ...
            echo "<td class='action-buttons'>
                  <div>
                       <a href='approve.php?id=$phno' class='edit'>Approve</a>
                       <a href='reject.php?id=$phno' class='delete'> Reject</a>
                  </div>
                  </td></tr>";
        }
    }
